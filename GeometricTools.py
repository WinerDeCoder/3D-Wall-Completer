...
def normalize_coordinate(points_tensor: [torch.Tensor]) -> [torch.Tensor]:
...
def center_object_tensor(points_tensor: [torch.Tensor]) -> [torch.Tensor]:
...
# def fps_feature(pc, num):
#     pc_xyz = pc[:,:,:3].contiguous()
    
#     fps_idx = pointnet2_utils.furthest_point_sample(pc_xyz, num) 
#     sub_pc  = pointnet2_utils.gather_operation(pc.transpose(1, 2).contiguous(), fps_idx).transpose(1,2).contiguous()
#     return dict(xyz = sub_pc[:,:,:3], feature = sub_pc[:,:,3:])

def center_object(self, points_tensor: Tensor) -> Tensor:
    max_point_x    = torch.max(points_tensor[:,0])
    max_point_y    = torch.max(points_tensor[:,1])
    max_point_z    = torch.max(points_tensor[:,2])
    min_point_x    = torch.min(points_tensor[:,0])
...
def normalize_coordinate(points_tensor: Tensor) -> Tensor:
    
    max_point    = torch.max(points_tensor, dim = 0)[0]
    min_point    = torch.min(points_tensor, dim = 0)[0]
    ...
    points_tensor_norm      = points_tensor.clone()
    points_tensor_norm      -= center
        
    return points_tensor_norm, center
